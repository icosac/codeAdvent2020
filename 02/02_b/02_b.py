...
def main():
    n_psw=0
    for l in range(len(input_l)):
        line=input_l[l]
        psw=line.split(': ')[1]
        info=line.split(': ')[0]
        C=info.split(" ")[1]
        m=min([int(i) for i in info.split(" ")[0].split("-")])-1
        M=max([int(i) for i in info.split(" ")[0].split("-")])-1
        if len(psw)>M and ((psw[m]==C and psw[M]!=C) or (psw[m]!=C and psw[M]==C)):
            n_psw+=1
        elif len(psw)<M:
            print(C, str(m), str(M), psw)
    print(n_psw)
