    ...
    for field in fields:
        ...
        if "hgt" in field:
            ...
            try:
                ...
                print("Exception: "+pssp+" <"+str(h.groups()[0])+">")
                return False

        if "hcl" in field and re.search("hcl:(#[\da-z]{6})", field)==None:
            return False
        if "ecl" in field:
            val=field.split(":")[1]
            if val not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
                return False
        if "pid" in field and re.search("pid:(\d{9})", field)==None:
            return False
    return True
...
def main():
    ...
    for i in range(len(input_str)):
        l=input_str[i]
        if l=="" or i==(len(input_str)-1):
            pssp+=l
            n=countC(pssp, ":")
            if n==8 and validate(pssp):
                valid+=1
            elif n==7 and "cid:" not in pssp and validate(pssp):
                valid+=1
            pssp=""
        else:
            pssp+=(l+" ")
